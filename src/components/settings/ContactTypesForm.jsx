...
import { contactTypeSchema } from '../../schemas/lookupSchema';

export function ContactTypesForm({ initialData, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = contactTypeSchema.safeParse(formData);
    ...
    try {
      ...
      if (isEditing) {
        await contactTypeApi.update(initialData.id, result.data);
      } else {
        await contactTypeApi.create(result.data);
      }

      onSuccess();
    } catch (err) {
      setError(getApiErrorMessage(err, 'Failed to save contact type.'));
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5 bg-slate-50/50">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              {isEditing
                ? 'Update communication channel details and policies.'
                : 'Define a new communication channel for employee profiles.'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Work Email, Mobile Phone, LinkedIn"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Specify standard validation or usage rules..."
            />
          </div>
