...
export function StatusesForm({ initialData, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await statusApi.update(initialData.id, result.data);
      } else {
        await statusApi.create(result.data);
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4 sm:p-6 transition-opacity">
      <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl transition-all border border-slate-100">
        {/* Modal Header */}
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5 bg-slate-50/50">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              Define the status name and unique system code.
            </p>
          </div>
          <button
            onClick={onClose}
